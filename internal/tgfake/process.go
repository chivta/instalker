...
// logPoll is how often the process output is re-read while waiting for a line.
const logPoll = 20 * time.Millisecond
...
// StartProcess launches binary with env layered on top of the current environment.
func StartProcess(binary string, env map[string]string) (*Process, error) {
